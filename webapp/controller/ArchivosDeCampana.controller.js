sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	return BaseController.extend("com.sap.build.standard.entregaSisemp.controller.ArchivosDeCampana", {
		...
		_onFioriObjectPageTableActionPress: function(oEvent) {
			...
			return new Promise(function(fnResolve) {
				...
				.then(function(result) {
					return new Promise(function(fnResolve) {
						sap.m.MessageBox.confirm("Al eliminar el archivo, ninguna otra persona tendra acceso. ¿Desea continuar?", {
		...
		_onIconPress: function() {
			return new Promise(function(fnResolve) {
				...
				sap.m.MessageToast.show("Descargando...", {
					onClose: fnResolve,
					duration: 2000 || 3000,
					at: sTargetPos,
					my: sTargetPos
			...
			if (status) {
				...
				return new Promise(function(fnResolve, fnReject) {
					...
					if (bHasPendingChanges) {
						...
						var oNewEntityInstance = Utilities.getDefaultValuesForCrearArchivo();
						oNewEntityInstance["___FK_0972231ebe507bd01b0530ee_00012"] = oSourceBindingContext.getProperty("ID");

						oModel = this.getView().getModel();
						var oNewBindingContext = oModel.createEntry("ArchivoSet", {
							properties: oNewEntityInstance
						});

						this.doNavigate("CrearArchivo", oNewBindingContext, fnResolve);
					}
				}.bind(this)).catch(function(err) {
					if (err !== undefined) {
						MessageBox.error(err.message);
		...
		onInit: function() {
			...
			this.oRouter.getTarget("ArchivosDeCampana").attachDisplay(jQuery.proxy(this.handleRouteMatched, this));
		...
		onExit: function() {
			...
			var aControls = [{
				"controlId": "Fiori_ObjectPage_ObjectPage_0-sections-Fiori_ObjectPage_Section-1-sectionContent-Fiori_ObjectPage_Table-1715638434697-39te9o2o5l2ut8krmny499tc6_S6",
				"groups": ["items"]
			}];
